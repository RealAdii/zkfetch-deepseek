 ...
 */
async function generateZkWitness({ algorithm, privateInput: { key }, publicInput: { ciphertext, iv, offset }, }) {
...
function getPublicSignals({ proof: { algorithm, plaintext }, publicInput: { ciphertext, iv, offset }, }) {
    const startCounter = (0, utils_1.getCounterForChunk)(algorithm, offset);
    const ciphertextArray = padCiphertextToChunkSize(algorithm, ciphertext);
...
async function decryptCiphertext({ algorithm, key, iv, offset, ciphertext, }) {
    const { chunkSize, bitsPerWord, encrypt } = config_1.CONFIG[algorithm];
